...
public class RuleSetCreator {
	 ...
	 * 
	 * The most important aspect of the project is the specification of the RuleSet by the clients 
	 * of this class.
	 * This is represented in the attribute this.inputSpec, which in turns is populated by the 
	 * constructor's input parameter inputSpec
	 * The format is as follows: inputSpec is a List of List<String>. Each internal List<String>  
	 * is a triplet of 3 strings:
	 ...
	 * 
	 * For example, a triplet saying "OMIT", "POSITIONS" "4,5" will omit the positions 4 and 5 
	 * in the list of paragraphs
	 * As another example, a triplet saying "H1", "ALL_CAPS" will treat all lines with all their 
	 * characters as capital letters as H1
	 * 
	 * @param lineblocks a List<LineBlock> representing the paragraphs of the input 
	 * (called LineBlocks in this project)
	...
	public RuleSetCreator(List<LineBlock> lineblocks, List<List<String>> inputSpec, String name) {
		...
		omitRule=currentRule; h1Rule=currentRule; h2Rule=currentRule; boldRule=currentRule; 
		italicsRule=currentRule;
	}
	
	/**
	 * The class populates a new RuleSet with all its necessary rules and returns it as the result 
	 * of the processing
	 * 
	 * The class employs the abstract class AbstractRule (to be concreted with concrete subclasses 
	 * per category of rule) and the respective Factory. Thus, a set of concrete classes to handle 
	 * the rules of AllCaps, Position, StartsWith, or Undefined are needed.
	 * At the end, the different kinds of rules of the RuleSet are all populated with an object of 
	...
	public RuleSet createRuleSet() {
		...
		for(List<String> l: inputSpec) {
			...
			switch(l.get(1).strip().toUpperCase()) {
				...
				case "POSITIONS":	lastParameter = l.get(2);
									List<String> stringList = new ArrayList<String>(
											Arrays.asList(lastParameter.split("\\s*,\\s*")));
									List<Integer> intList = stringList.stream().map(
			...
			case "OMIT":	omitRule = currentRule;
							System.out.println("[RuleSetCreator] OMIT: " + currentRule.toString()+"\n");
							break;
			case "H1":		h1Rule = currentRule;
							System.out.println("[RuleSetCreator] H1: " + currentRule.toString()+"\n");
							break;
			case "H2":		h2Rule = currentRule;
							System.out.println("[RuleSetCreator] H2: " + currentRule.toString()+"\n");
							break;
			case "<B>":		boldRule = currentRule;
							System.out.println("[RuleSetCreator] <B>: " + currentRule.toString()+"\n");
							break;
			case "<I>":		italicsRule = currentRule;
							System.out.println("[RuleSetCreator] <I>: " + currentRule.toString()+"\n");
							break;
			default:		System.err.println("[RuleSetCreator] Wrong rule set specification syntax. "
												+ "Aborting");
							System.exit(-100);
			}
