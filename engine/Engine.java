...
public class Engine implements IPlainTextDocumentEngine {
	...
	// PK: mporw na valw xeri sto Engine an xreiastei
	 ...
	 * @param pInputType the String characterizing the document as raw (DocumentRawType.RAW) unless 
	...
	public Engine(String pFilePath, String pInputType, String pAlias) {
		...
		this.document = new Document(pFilePath, docType, alias);
		this.lineblocks = null;
		//this.lineblocks = this.document.getLineblocks();				// FIXME - remove from here
		this.alias = pAlias;
	 ...
	 * 
	 * Returns a RuleSet object as the result of the parsing and internal representation 
	 * of the rules expressed as strings.
	 ...
	 * Every category absent is setup to undefined status and eventually mapped to the 
	 * respective RuleUndefined object that always returns false for isValid()
	 * The 0th element is the aforementioned category representative string
	 * The 1st element is STARTS_WITH, POSITIONS, ALL_CAPS 
	 * The 2nd element is a prefix string for STARTS_WITH or a comma-separated string of positions 
	 * (starting from 0) for POSITIONS
	 * 
	 * @param inputSpec a List of (List of String) with the specification of the rules on 
	 * how to handle paragraphs
	 * @return a RuleSet object as the result of the parsing and internal representation of 
	 * the rules expressed as strings.
	 */
	@Override
	 ...
	 * 
	 * Returns a RuleSet object as the result of the parsing and internal representation of 
	 * the rules expressed as strings.
	 * The inputSpec parameter representing the specification of the rules is a List of (List of String)
	 * Each category of rule (OMIT, H1, H2, <B>, <I>) has a dedicated (List of String). 
	 * Every category absent is setup to undefined status and eventually mapped to the 
	 ...
	 * The 2nd element is a prefix string for STARTS_WITH
	 * 
	 * The prefixes parameter represents the List<String> to report on the marks at the beginning of 
	 * each paragraph.
	 *  
	 * @param inputSpec a List of (List of String) with the specification of the rules on how to 
	 * handle paragraphs
	 * @param prefixes a List of Strings as the prefixes for the annotated paragraphs of the file
	 * @return a RuleSet object as the result of the parsing and internal representation of the 
	 * rules expressed as strings.
	 */
	@Override
...
//			prefixes.add(l.get(2));
	...
	/**
	 * Takes the input file specified at the constructor, loads it and processes it according to 
	 * the rule set specified at the constructor
	 *  
	...
	/**
	 * Exports the input file of the constructor as the MarkDown file at the path specified 
	 * by outputFileName
	 * 
	 *  If the input files has not been processed, and the this.lineblocks attribute has a size of 0, 
	 *  the method loads and characterizes the input
	 *  	
	...
	public int exportMarkDown(String outputFileName) {
		...
		System.out.println("[Engine.loadProcessMarkup] [file: " + simpleInputFileName + "] exported as " 
		+ outputFileName +"\n Input #pars: " + this.lineblocks.size() + " Output #pars: " 
		+ outputNumParagraphs);
	...
	public int exportPdf(String outputFileName) {
		...
		int outputNumParagraphs = exporter.export();
		System.out.println("[Engine.loadProcessPdf] [file: " + simpleInputFileName + "] Input #pars: " 
		+ this.lineblocks.size() + " Output #pars: " + outputNumParagraphs);
	...
	private void characterizeLineblocks(Document document, RuleSet ruleSet) throws IOException {
		//List<LineBlock> lineblocks = document.getLineblocks();	// FIXME Has already been done in constructor!!!!!
